...
async function main() {
	...
	const token1 = await Token.deploy('Decentratality_crowdsale', SYMBOL, MAX_SUPPLY)
	const token2 = await Token.deploy('Decentratality_DAO', SYMBOL, MAX_SUPPLY)
	

	
	console.log(`Crowdsale Token deployed to: ${token1.target}\n`)
	console.log(`DAO Token deployed to: ${token2.target}\n`)

	const Crowdsale = await hre.ethers.getContractFactory('Crowdsale')
	const crowdsale = await Crowdsale.deploy(token1.target, PRICE, ethers.parseUnits(MAX_SUPPLY, 'ether'))
	

	console.log(`Crowdsale deployed to: ${crowdsale.target}\n`)

	let transaction = await token1.connect(funder).transfer(crowdsale.target, ethers.parseUnits(MAX_SUPPLY, 'ether'))
	await transaction.wait()

	console.log('Tokens transferred to Crowdsale')

	const DAO = await hre.ethers.getContractFactory('DAO')
  	const dao = await DAO.deploy(token2.target, '500000000000000000000001')
